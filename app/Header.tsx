...
export default function Header() {
  ...
  return (
    <header className="flex sticky top-0 w-full z-50 header-glass header">
      <div className="h-16 flex items-center justify-around px-4 md:px-8 w-full">
        {/* Logo */}
      ...
      {menuOpen && (
        <div className="absolute top-14 left-0 w-full action-lang border-t border-current/10 flex flex-col gap-1 px-4 py-3 md:hidden z-50">
          <div className="min-w-10 justify-center flex group relative py-2" onMouseLeave={() => setIsLangOpen(false)}>
            <span
              onClick={(e) => { e.stopPropagation(); setIsLangOpen(!isLangOpen); }}
            ...
            <div className={`w-full absolute top-full pt-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible group-hover:translate-y-0 ${isLangOpen ? '!opacity-100 !visible' : ''}`}>
              <div className="action-lang flex flex-col gap-2 rounded-xl min-w-[60px] text-center">
                {(['ukr', 'rus', 'eng', 'esp', 'est'] as const).map((l) => (
                  <span
                    key={l}
                    onClick={(e) => {
