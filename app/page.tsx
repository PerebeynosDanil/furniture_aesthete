      ...
      <main className="flex-grow flex flex-col items-center w-full">
        ...
        <FadeIn>
          <div className="banner relative w-full overflow-hidden shadow-2xl">
            ...
            <div className="banner-overlay bg-[var(--bg-overlay)] top-0 z-1 flex justify-center ">
              <div className='presentation-author flex flex-col md:flex-row justify-center items-center px-2 gap-4 md:gap-10 py-2 md:py-0'>
                <img src="/img/author-photo.jpg" className="object-cover object-[50%_15%] w-32 h-32 md:w-auto md:h-auto rounded-full md:rounded-none" />
                ...
                <div className="grid grid-cols-2">
                  {cur.facts.map((fact: any, i: number) => (
                    <div key={fact.num} className={`p-4 md:p-5 ${i % 2 === 0 ? 'border-r border-white/10' : ''} ${i < 2 ? 'border-b border-white/10' : ''}`}>
                      <span className="block text-[16px] tracking-widest text-white mb-2 md:mb-3">{fact.num}</span>
                      <div className="h-px bg-white/15 mb-2 md:mb-3" />
                      <p className="text-[16px] font-medium text-white mb-1 md:mb-2">{fact.title}</p>
                      <p className="text-[14px] text-white/60 leading-relaxed">{fact.desc}</p>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <FadeIn>
          <div className="banner relative w-full">
            <img src="/img/banner-second.jpg" className="h-[36em] w-full object-cover" />
            <div className="banner-overlay w-full h-full  mx-auto flex flex-col justify-center items-center">
              <div className='max-w-5xl px-4 md:px-6 py-2 md:py-12'>
                <h2 className="text-2xl md:text-3xl font-medium mb-1">{cur.servicesTitle}</h2>
                <p className="text-base mb-6 md:mb-8">{cur.servicesSubtitle}</p>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6 ">
                  {cur.services.map((service: any, i: number) => (
                    <div key={i} className='bg-[var(--bg-second)] p-3 rounded-2xl'>
                      <p className="text-[16px] font-medium mb-1">{service.title}</p>
                      <p className="text-[14px] leading-relaxed">{service.desc}</p>
                    </div>
                  ))}
                </div>
              </div>
        ...
        {/* ── BRANDS ── */}
        <FadeIn>
          <div className="min-[530px]:w-md md:w-full max-w-5xl mx-auto px-4 md:px-6 py-8 md:py-12">
            <h2 className="text-center text-2xl md:text-3xl font-medium mb-2">{cur.brandsTitle}</h2>
            <p className="text-center text-base mb-8 md:mb-10">{cur.brandsSubtitle}</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 md:gap-8">
              {cur.brands.map((brand: any, i: number) => (
                <div key={i}>
                  ...
                  <p className="text-[16px] font-medium mb-1">{brand.name}</p>
                  <p className="text-[14px] leading-relaxed" dangerouslySetInnerHTML={{ __html: brand.desc }} />
                </div>
              ))}
            </div>
          </div>
        ...
        {/* ── CERT ── */}
        <FadeIn>
          <div className="min-[530px]:w-md md:w-full max-w-5xl mx-auto px-4 md:px-6 py-8 md:py-12 bg-[var(--bg-second)] rounded-2xl mb-4 md:mb-6">
            ...
            <div className="flex flex-col md:flex-row gap-6 md:gap-10 items-center">
              <div className="w-full md:w-[420px] shrink-0 rounded-2xl overflow-hidden cursor-pointer group" onClick={() => setSelectedDoc('/img/diploma.jpg')}>
                <img src="/img/diploma.jpg" alt="Puustelli diploma" className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105 aspect-[4/2] md:aspect-[4/6]" />
              </div>
        ...
        {/* ── BVS ── */}
        <FadeIn>
          <div className="w-full px-4 md:px-6 py-8 md:py-12 bg-[var(--bg-second)] rounded-2xl">
                ...
                <div className="flex-1 w-full flex flex-col gap-4 ">
                  {cur.bvsCards.map((card: any, i: number) => (
                    <div key={i} className="border border-gray-700 rounded-2xl px-5 md:px-6 py-4 md:py-5 bg-[var(--bg-second)]">
                      ...
                      <p className="text-[14px] leading-relaxed">{card.desc}</p>
                    </div>
                  ))}
                </div>
              </div>
        ...
        {/* ── Furniture Aesthete ── */}
        <FadeIn>
          <div className="min-[530px]:w-md md:w-full max-w-4xl mx-auto px-4 md:px-6 py-8 md:py-12">
            <div className="flex flex-col md:flex-row gap-6 md:gap-10 items-stretch">
              <div className="flex-1 flex flex-col gap-4">
                <h2 className="text-2xl md:text-3xl font-medium mb-2 text-center">{cur.logo}</h2>
            ...
            <div className="max-w-5xl mx-auto px-4 md:px-6 py-8 md:py-12 bg-[var(--bg-second)] rounded-3xl">
              ...
              <div className="flex flex-col md:flex-row gap-6 md:gap-10">
                ...
                <div className=" flex-1 flex flex-col gap-4 md:gap-5">
                  <p className="text-2xl font-medium">{TIKTOK_USERNAME}</p>
                  <div className="space-y-3 text-[16px] leading-relaxed">
                    {cur.tiktokText.map((p: string, i: number) => <p key={i}>{p}</p>)}
                  </div>
                  <a href={TIKTOK_PROFILE_URL} target="_blank" rel="noopener noreferrer" className=" inline-flex items-center gap-2 w-fit px-6 py-3 bg-[var(--bg-action)] text-[var(--text-action)] font-medium rounded-xl hover:opacity-80 transition-opacity text-base">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4">
                {cur.ctaCards.map((card: any, i: number) => (
                  <div key={i} className="border border-gray-700 rounded-2xl px-4 py-4" style={{ background: 'var(--bg-second)' }}>
                    <p className="flex items-center gap-2 text-[16px] font-medium mb-1">{CTA_ICONS[i]}{card.title}</p>
                    <p className="text-[14px] leading-relaxed">{card.desc}</p>
                  </div>
                ))}
              </div>
              <div className="text-center">
                <button onClick={() => setContactOpen(true)} className="px-6 py-3 bg-[var(--bg-second)] cursor-pointer font-medium rounded-xl hover:opacity-80 transition-opacity border border-gray-700 text-base">
                  {cur.ctaBtn}
                </button>
              </div>
            </div>
