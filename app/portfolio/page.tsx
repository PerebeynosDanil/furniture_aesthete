    ...
    <div className="min-h-screen flex flex-col" style={{ background: 'var(--bg-main)' }}>
      ...
      <FadeIn>
        <div className="relative w-full h-[36vh] md:h-[44vh] overflow-hidden flex items-center">
          <img src="/img/portfolio-banner.jpg" alt="Portfolio" className="absolute inset-0 w-full h-full object-cover bg-[]" />
          <div className="bg-[var(--bg-overlay)] rounded-2xl z-10 w-fit max-w-5xl mx-auto px-4 py-4 md:px-6 pb-10">
        ...
        <div className="w-full bg-[var(--bg-second)] py-7 px-4">
          <div className="max-w-3xl mx-auto flex items-center gap-4">
            <button
              ...
              className="shrink-0 w-8 h-8 rounded-full border border-gray-700 flex items-center justify-center hover:bg-gray-500 transition-colors cursor-pointer"
            >
              <ChevronLeft size={16} />
            </button>
      ...
      {modal && (
        <div
          className="group fixed inset-0 z-[200] flex items-center justify-center bg-black/90 backdrop-blur-sm p-4 select-none"
          ...
          onTouchEnd={() => {
            ...
            if (isLeftSwipe && modal.project.photos.length > 1) {
              modalCycle(1);
            }
            if (isRightSwipe && modal.project.photos.length > 1) {
              modalCycle(-1);
            }

            // Сбрасываем стейты после завершения свайпа
            setTouchStart(null);
            setTouchEnd(null);
          }}
        >
          <button className="absolute cursor-pointer top-4 right-4 text-white hover:opacity-70 transition-opacity z-10" onClick={() => setModal(null)}>
            <X size={36} />
          </button>

          {modal.project.photos.length > 1 && (
            <button
              className="absolute cursor-pointer left-1 md:left-6 text-white md:bg-black/40 md:hover:bg-black/70 rounded-full w-11 h-11 flex items-center justify-center z-10"
          ...
          {modal.project.photos.length > 1 && (
            <button
              className="absolute cursor-pointer right-1 md:right-6 text-white md:bg-black/40 md:hover:bg-black/70 rounded-full w-11 h-11 flex items-center justify-center z-10"
              onClick={(e) => { e.stopPropagation(); modalCycle(1); }}
            >
